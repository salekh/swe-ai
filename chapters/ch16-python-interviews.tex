...
class RateLimiter:
    ...
    def allow(self, user_id: str, tokens: int = 1) -> bool:
        # Check global limit first (non-destructive peek)
        with self._lock:
            user_bucket = self._users[user_id]
        # Attempt global first --- if global rejects,
        # user tokens are never deducted.
        if not self._global.consume(tokens):
            return False
        # Global accepted; now check per-user limit.
        if not user_bucket.consume(tokens):
            # Refund the global tokens we just consumed.
            with self._global._lock:
                self._global._tokens = min(
                    self._global.capacity,
                    self._global._tokens + tokens)
            return False
        return True
\end{lstlisting}
...
class CircuitBreaker:
    def __init__(self, failure_threshold: int = 5,
        ...
        self._half_open_permitted = True  # atomic probe flag
        self._lock = threading.Lock()
    ...
    def call(self, func, *args, **kwargs):
        ...
        with self._lock:
            ...
            if current_state == State.HALF_OPEN:
                # Atomic test-and-set: only ONE request
                # probes the service; others are rejected.
                if not self._half_open_permitted:
                    raise RuntimeError(
                        "Circuit HALF_OPEN: probe in flight.")
...
class SemanticCache:
    def __init__(self, embed_fn: Callable[[str], np.ndarray],
        ...
        # Map key -> index in _emb_matrix for O(1) lookup.
    ...
    def get(self, query: str) -> str | None:
        ...
        emb_matrix = np.stack(self._emb_list)
        similarities = self._cosine_sim(query_emb, emb_matrix)
        best_idx = int(np.argmax(similarities))
        best_sim = similarities[best_idx]

        if best_sim >= self._threshold:
            # Reverse-lookup key from index.
            key = list(self._key_to_idx.keys())[
                list(self._key_to_idx.values()).index(best_idx)]
    ...
    def put(self, query: str, response: str,
        ...
        self._key_to_idx[query] = idx
        self._emb_list.append(embedding)

    def _evict(self, key: str) -> None:
        idx = self._key_to_idx.pop(key)
        del self._cache[key]
        # Swap-and-pop: O(1) removal from embedding list.
        last_idx = len(self._emb_list) - 1
        if idx != last_idx:
            self._emb_list[idx] = self._emb_list[last_idx]
            # Update the swapped key's index.
            for k, v in self._key_to_idx.items():
                if v == last_idx:
                    self._key_to_idx[k] = idx
                    break
        self._emb_list.pop()
