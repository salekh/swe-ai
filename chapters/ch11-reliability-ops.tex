...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Native prompt caching}: All major providers cache KV-cache states for stable prompt prefixes, giving 50--90\% input token discounts. Structure prompts with stable content (system prompt, few-shot examples) first and variable content (user query) last to maximize cache hits.
    \item \textbf{Semantic caching}: Application-level semantic caching (Chapter~\ref{ch:app-architecture}) reduces API calls by 20--40\% for repetitive query patterns.
